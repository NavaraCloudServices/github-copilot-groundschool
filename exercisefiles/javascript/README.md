[![Open in GitHub Codespaces](https://github.com/codespaces/badge.svg)](https://codespaces.new/NavaraCloudServices/github-copilot-groundschool/tree/main?devcontainer_path=.devcontainer%2Fjavascript-node%2Fdevcontainer.json)

# Goal

The goal is to create a server using NodeJS with the help of GitHub Copilot. 
...
- Open a command prompt and run the test (`npm run test` or `mocha test.js`)
...
You will be implementing the following additional features using the `requests` package:
- [ ] DaysBetweenDates
- [ ] ValidatePhoneNumber
- [ ] ValidateSpanishDNI
...
- **/ValidateSpanishDNI**:
  ...
  > **_NOTE:_** Use above information inside a comment in the `nodeserver.js` file. In this case, you may want to see multiple solutions from Copilot to pick the one that best fits the way to calculate the letter. In order to see the firs 10 suggestions from Copilot press `ctrl + enter`.
...
- In the chat, ask Copilot to document the `nodeserver.js` file by including it using `#nodeserver.js`.
